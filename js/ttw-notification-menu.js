        ...
        NotificationList.prototype.bindMenuActions = function() {
            ...
            this.$wrapper.delegate('.close', 'click', function() {
              ...
              //send request to CODS, mark the notification as 'readed'
                $.getJSON("requestProcessCenter.php?type=3&&id="+$this.prev("span[class='message']").children("input").val(), function (result, textStatus){
                	//actions to be done.
                	alert(result);
    				});
            });
            
          //handle clicks of decline
            this.$wrapper.delegate('.requestDecline', 'click', function() {
                var $this = $(this);
                ...
                //send request to CODS, make it as 'declined'
                $.getJSON("requestProcessCenter.php?type=2&&id="+$(this).children("input").val(), function (result, textStatus){
                	//actions to be done.
                	alert(result);
    				});
            });
          //handle clicks of accept
            this.$wrapper.delegate('.requestAccept', 'click', function() {
                var $this = $(this);
                ...
                //send request to CODS. mark it as 'accepted'
                $.getJSON("requestProcessCenter.php?type=1&&id="+$(this).children("input").val(), function (result, textStatus){
    				//add the reminder by createNofitications
                	alert(result);
    					/*var options = {
    			               	category:'reminder',
