     ...
     $.ttwNotificationMenu = function(userOptions) {
        ...
        NotificationList.prototype.bindMenuActions = function() {
            ...
            this.$wrapper.delegate('.close', 'click', function() {
                var $this = $(this);
                var notification_id=$this.parent().children('.message').children("#span_for_notification_id").children('input:hidden').val();
                //****************this is done by Yu Song,which is used to send notification_id
                //****************to the CODS server to delete notifications
                $.ajax({
                	type: "POST",
                	url: "notification_controller/delete_notification.php",
